...
class QueueManager:
    ...
    async def _recompute_pending_from_now(self, instance_id: str, now_s: Optional[float] = None) -> None:
        ...
        async with lock:
            ...
            # Rebuild reservation chain from real "now" baseline while preserving
            # already-reserved worker release timeline.
            slot_free = [max(ts_s, v) for v in self._instance_slot_free_ts_s[instance_id]]
