...
class QueueManager:
    ...
    @staticmethod
    def _predict_decode_for_stage_s(task: ProxyTask, start_length: int, decode_bs: int = 3) -> float:
        stage = str(getattr(task, "predict_stage", "prefill") or "prefill").lower()
        base_length = max(1, int(start_length))
        if stage == "prefill":
            # Prefill主链仅保留首token尾部开销（1 token decode tail）。
            return max(0.0, decode_tpot_predictor(length=base_length, bs=decode_bs))
        if stage == "decode":
            # Decode阶段接口保留，当前仍可复用原累计逻辑（后续会替换为动态bs跟踪）。
            return QueueManager._predict_decode_total_s(task, start_length=base_length, decode_bs=decode_bs)
        return max(0.0, decode_tpot_predictor(length=base_length, bs=decode_bs))
    ...
    async def _mark_task_first_token_and_recompute(self, task: ProxyTask, instance_id: str) -> None:
        ...
        async with lock:
            ...
            # first_token only releases shared prefill timeline.
    ...
    async def _mark_task_forward_end_and_recompute(self, task: ProxyTask, instance_id: str) -> None:
        now_s = time.time()
        lock = self._get_reservation_lock(instance_id)
        async with lock:
            self._ensure_instance_reservation_state(instance_id, now_s=now_s)
            task.has_finished_forward = True
            self._instance_pending_tasks[instance_id] = [
                t for t in self._instance_pending_tasks[instance_id]
                if t is not task
            ]
