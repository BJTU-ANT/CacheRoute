...
CacheRoute是一种新型的跨LLM系统任务调度平台。它考虑到AI的现有检索式业务（如浏览器AI、知识问答AI）涉及大量知识重用，通过动态调整知识注入策略来有效提升任务性能和系统吞吐量。具体内容见xxx。

### 架构
---------------------------------------------------------------------------------<br>
| [Client] -> [Scheduler] -> [Proxy] -> [Instance] -> vLLM实例 |<br>
---------------------------------------------------------------------------------<br>
- Client以http请求格式发起推理服务，并发送给Scheduler。<br>
- Scheduler收到请求后会解析请求信息并构建Request类。其中涉及调用KDN知识库和调度方法。然后基于调度策略得到的IP地址，发送给指定的本地代理Proxy
- Proxy接收到请求后，根据本地策略送入任务队列等待。直到满足任务下发条件后将http请求移交instance
