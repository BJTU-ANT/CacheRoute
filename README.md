...
4. 在`core/config.py`内根据实际模型下载路径完成必要的参数配置（scheduler强依赖embedding、tokenizer、model模型）
   ...
   4.2 为实现跨容器KVCache复用，需要抛弃`builtin+SEED`的不稳定KEY生成方法，采用`sha256_cbor`方法，但由于output格式不对齐问题，CacheRoute对`token_database.py`进行了补丁更新。因此需要将lmcache源码中的`lmcache/v1/token_database.py`文件替换为`CacheRoute/env/token_database.py`
