...
class TokenDatabase(metaclass=abc.ABCMeta):
    ...
    def _normalize_hash_value(self, hv: Any) -> int:
        """
        Normalize hash function outputs from different vLLM versions / algorithms
        into a stable Python int for CacheEngineKey.chunk_hash.

        Rules:
        - int      -> keep as uint64-style positive int
        - bytes    -> take first 8 bytes as big-endian unsigned integer
        - str      -> parse as hex/int if possible, otherwise utf-8 bytes -> first 8 bytes
        """
        ...
        if isinstance(hv, bytes):
            if len(hv) < 8:
                hv = hv.ljust(8, b"\x00")
            return int.from_bytes(hv[:8], byteorder="big", signed=False)
        ...
        if isinstance(hv, str):
            ...
            try:
                ...
                if len(bs) < 8:
                    bs = bs.ljust(8, b"\x00")
                return int.from_bytes(bs[:8], byteorder="big", signed=False)

        raise TypeError(
            f"Unsupported hash value type for CacheEngineKey.chunk_hash: {type(hv)}"
        )
    ...
    def _hash_tokens(
        ...
        raw_hash = self.hash_func((prefix_hash, tokens_tuple, extra_keys))
